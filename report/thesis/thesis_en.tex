...
\subsection{Titans}
While Transformers have achieved unprecedented success across various domains, their standard architecture suffers from two fundamental limitations: the quadratic computational complexity of the attention mechanism with respect to sequence length ($\mathcal{O}(N^2)$), which severely bounds the maximum context window, and the static nature of their weights during inference. Because a trained Transformer's weights are frozen, it cannot learn from new context dynamically; it merely caches tokens in its finite working memory.

To address these bottlenecks, the Titans architecture \parencite{behrouzTitansLearningMemorize2024} departs from relying exclusively on self-attention by introducing a dynamic, biologically inspired Neural Memory System. Rather than forcing a single attention mechanism to process the entirety of a sequence, the Titans architecture partitions the cognitive workload across three distinct memory modules:

\begin{description}
	\item[Short-Term Memory] This module operates identically to the traditional Transformer attention mechanism. It serves as the model's immediate mental workspace, providing high-resolution, precise processing strictly for the local, immediate context.
	
	\item[Long-Term Memory] This is the core innovation of the architecture. Instead of attempting to scale the context window to infinity, Titans introduce a secondary neural network whose weights are dynamically updated \textit{during inference} (test time). This module compresses historical context into neural weights rather than caching explicit token representations.
	
	\item[Persistent Memory] This tier consists of learnable, static parameters that encode the fundamental rules and meta-knowledge of the task itself, ensuring the model does not suffer from catastrophic forgetting while its Long-Term Memory aggressively adapts to novel prompts.
\end{description}

\subsubsection*{Mathematical Formulation of the Neural Long-Term Memory}

The foundational mathematics of the Titans architecture redefines long-term memory not as a static cache of tokens, but as an online optimization problem. The independent neural network ($\mathcal{M}$) governing this memory adapts its parameters at test time via gradient descent, driven by an associative loss function and a biologically inspired "surprise" metric.

\par The long-term memory module aims to learn associations between historical keys ($k_t$) and values ($v_t$) generated from the input sequence. The memory network $\mathcal{M}$, parameterized by weights at time step $t-1$ ($W_{t-1}$), attempts to reconstruct the value given the key: $\mathcal{M}(k_t; W_{t-1}) \approx v_t$. The accuracy of this reconstruction is measured by an associative loss function, typically defined as Mean Squared Error (MSE):

\begin{equation}
	\mathcal{L}_t = \left\| \mathcal{M}(k_t; W_{t-1}) - v_t \right\|^2
\end{equation}

\par Drawing from cognitive science, Titans posit that highly predictable data should require little memorization, whereas unexpected data requires structural changes in the neural pathways. Mathematically, "surprise" is quantified as the gradient of the associative loss with respect to the memory parameters. If the memory module flawlessly predicts the value (low loss), the gradient is near zero (low surprise). Conversely, a failed prediction yields a large gradient (high surprise):

\begin{equation}
	S_t = \nabla_{W_{t-1}} \mathcal{L}_t
\end{equation}

\par Instead of remaining frozen after training, the memory module updates its parameters sequentially during inference. To prevent memory overflow and catastrophic forgetting, the update mechanism incorporates a decay factor ($\alpha$) for past gradients and a learning rate ($\theta$) for the momentary surprise ($S_t$). This creates a momentum-based update rule:

\begin{equation}
	W_t = W_{t-1} - \left( \alpha \Delta W_{t-1} + \theta S_t \right)
\end{equation}

where $\Delta W_{t-1}$ represents the accumulated historical surprise. By embedding this test-time gradient descent directly into the forward pass, the Titans architecture effectively compresses the entirety of the sequence history into the deep weights of $W_t$. When a new query ($q_t$) arrives, the model simply queries this dynamically updated network ($\mathcal{M}(q_t; W_t)$) to retrieve context, entirely circumventing the $\mathcal{O}(N^2)$ dot-product computations required by standard attention mechanisms.

\subsection{Mamba}
While the Titans architecture addresses the context bottleneck by compressing history into dynamic weights, an alternative and highly successful paradigm has emerged in the form of Structured State Space Sequence Models (SSMs), most notably the Mamba architecture \parencite{guMambaLinearTimeSequence2024}. Mamba completely discards the attention mechanism, offering a fundamental architectural shift that scales linearly in time ($\mathcal{O}(N)$) with respect to sequence length, while maintaining a constant memory footprint during autoregressive generation.
