...
%
\documentclass[en,ack]{dimscthesis}
...
\repourl{https://github.com/RafailTn/msc-thesis/tree/main}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%% Required Metadata %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
\boardThreeGr{Αλέξανδρος Δημόπουλος}
\boardThreeRankGr{Επίκουρος Καθηγητής}
\boardThreeOrgGr{Χαροκόπειο Πανεπιστήμιο}

\boardThreeEn{Alexandros Dimopoulos}
\boardThreeRankEn{Assistant Professor}
\boardThreeOrgEn{Harokopio University}
...
\supervisorsGr[Επιβλέποντες]{
    {\bfseries Martin Reczko}, Καθηγητής BSRC-Alexander Flemming 
}

\supervisorsEn[Supervisors]{
    {\bfseries Martin Reczko}, Professor BSRC-Alexander Flemming 
}
...
\par Before cross-linking via UV irradiation, AGO expression is induced via the use of Doxicyclin, where the concentration is monitored in order for AGO to be close to endogenous levels. Despite that, differences between endogenous expression do occur and with them, the risk of bias regarding specific interactions. After cross-linking, hydrolysis of interacting RNA molecules takes place followed by ligation of the hybridized miRNA:MRE duplex which creates hybrid molecules to be reverse transcribed, amplified and sequenced. AGO-eCLIP follows a different procedure that does not require modifications to the endogenous expression. During AGO-eCLIP, the RISC:MRE complex is cross-linked using UV irradiation and the RNA is fragmented in the same manner as with AGO-CLASH, then immunoprecipitation, using antibodies that bind the AGO protein, follows and ultimately ligation of the mRNA 5'-P to the miRNA's 3'-OH group \parencite{manakovScalableDeepProfiling2022}. Due to the immunoprecipitation step, no exogenous induction of expression is needed, eliminating the aforementioned bias and providing an image that is more physiologically relevant.
...
\par Some modern approaches like the one from \cite{zacharopoulouMicroTCNNAvantgardeDeep2024} employ such architectures combining them with ones like Gated Recurrent Units (GRUs) in order to extract a unified score across an entire gene and use it as a feature for a meta learner that is trained on expression data to predict functional binding which goes beyond site level predictions. The most recent and advanced models perform what is called Pathway Level prediction, by taking advantadge of site/gene level predictors and Graph Neural Networks (GNNs), they predict pathway level interactions between mRNAs-miRNAs-Competing Endogenous RNAs (ceRNAs), making them more relevant for identifying which of these pathways are crucial for disease states \parencite{chiuCupidSimultaneousReconstruction2015,jinTransparentSparseGraph2024}. Finally, modern models may also rely on Transformer based architectures which are able to treat DNA/RNA as a language and learn more intricate pattern that are not necessarily in proximity in the input sequence. Examples of these include the vanilla Transformer, Titans and the Mamba architecture which is not transformer based but has shown promise in DNA related tasks \parencite{behrouzTitansLearningMemorize2024,guMambaLinearTimeSequence2024}
...
Optuna \parencite{akibaOptunaNextgenerationHyperparameter2019} was used in order to perform tuning for 5 different hyperparameters, setting the Validation set APS as the score to maximize:

\begin{enumerate}
	\item Learning Rate (\numrange{1e-5}{1e-2}), Best: \num{2e-3} 
	\item Weight Decay (\numrange{1e-6}{1e-2}), Best: \num{2e-4} 
	\item Hidden Dimensions (\numrange{32}{128}), Best: \num{32}
	\item Number of Mamba Layers (\numrange{1}{3}), Best: \num{3}
	\item Dropout (\numrange{0.1}{0.3}), Best: \num{0.15}, changed to \num{0.3} to mitigate overfitting
\end{enumerate}

Additionally, another configuration for the Mamba architecture was tested that explicitly incorporates reverse-complement (RC) information, motivated by the antiparallel nature of miRNA–MRE binding. Because Mamba is an inherently unidirectional state-space model, it processes the input strictly left-to-right, it cannot attend to downstream context the way a Transformer can, making it unable to naturally capture the antiparallel complementarity that governs miRNA–target interactions. To address this, rather than stacking two separate Mamba encoders in opposite directions (a common bidirectional design), the model constructs a single extended sequence that presents both strand orientations within one forward pass. Specifically, the four segments are concatenated as:

\begin{center}
MRE $\mid$ \texttt{[SEP]} $\mid$ miRNA $\mid$ \texttt{[SEP]} $\mid$ rc(miRNA) $\mid$ \texttt{[SEP]} $\mid$ rc(MRE)
\end{center}

where \texttt{[SEP]} is a zero-vector separator token that signals a segment boundary to the Mamba hidden state without carrying any sequence-specific information. The reverse complement of each integer-encoded sequence is computed by swapping A$\leftrightarrow$T and C$\leftrightarrow$G and then reversing the sequence, so that the canonical antiparallel orientation of the duplex is made directly readable by the left-to-right scan. By the time the model reaches the end of the concatenated sequence, the Mamba state has accumulated information from the MRE in both orientations and from the miRNA in both orientations, making cross-strand complementarity patterns accessible through the recurrent state rather than through attention.

The sequence encoding was changed from one-hot to integer indices (A=0, C=1, G=2, T/U=3, unknown=4, padding=5), fed through a learnable embedding layer of dimension $d_{\text{model}}=128$ with a dedicated padding index. The backbone is otherwise identical to the earlier Mamba variant: stacked residual blocks each applying pre-LayerNorm followed by a Mamba-2 layer and dropout, then a final LayerNorm and an attention-based pooling layer that produces a fixed-size representation while explicitly ignoring padded positions via a boolean mask. Padding masks for the RC segments are reused from their forward counterparts since RC does not alter sequence length. Training followed the same 5-fold miRNA-family-stratified cross-validation protocol, with early stopping on validation Average Precision (patience 7) and a CosineAnnealing learning rate schedule. Unfortunately, this configuration did not yield improved performance over the earlier Mamba variant, and systematic hyperparameter tuning for this model remains to be attempted.
...
Energy and duplex related features were extracted from scratch with the addition of a parameter in IntaRNA that disables the accessibility computation for miRNA sequences, given that they are bound by AGO proteins and are therefore not folded while additional conservation features were added describing more aspects of the distribution of these scores locally across the sequence of the MRE. Feature selection was performed via Featurewiz which led to a reduction of 190 features in total to 58 with minimal losses in performance. 
...
\begin{table}[H]
    % this is before the table
    \caption{Final Model vs miRBind vs TargetScan}
    % this is to label and include it in our list of tables
    \label{tbl:Model Comparison}
    \begin{center} % center the table
        \begin{tabular}{c|c|c|c|c}
		\textbf{Evaluation} & \textbf{TargetScan} & \textbf{miRBind} & \textbf{miRBind2}& \textbf{Final Model} \\
            \hline
...
\par In order to interpret the predictions during inference, the model was used to predict samples in the leftout dataset and the calculation of Shapley values followed. In order to avoid calculating for thousands and possibly millions of samples depending on the dataset, K-Means is used in order to group the samples within the feature space in one of 25 clusters. Then, instead of calculating the Shapley value using all of the different sample combinations as background (thus performing background calculation as many times as samples), it does so, based on the number of cluster representatives. A list of the top 30 features ranked based on the mean of their importance across 200 samples is shown below as well as PFI based on all of the samples of the same file:

\begin{table}[H]
    \centering
    \caption{Global Feature Importance (Mean Absolute SHAP Values) - Complete Results}
    \label{tbl:feature_importance_complete}
    \footnotesize
    \begin{tabular}{llr}
        \toprule
        \textbf{Rank} & \textbf{Feature} & \textbf{Mean Abs SHAP} \\
        \midrule
        1 & Eall & 0.150368 \\
        2 & seed\_matches\_2\_8 & 0.112795 \\
        3 & gu\_wobbles\_in\_seed\_2\_8\_pos & 0.033197 \\
        4 & mre\_5p\_energy\_mean & 0.019582 \\
        5 & total\_mre\_bulges & 0.018676 \\
        6 & downstream\_gini & 0.014208 \\
        7 & Eall1 & 0.013175 \\
        8 & binding\_type\_6mer.mismatch.3prime & 0.011076 \\
        9 & seed\_conservation\_min & 0.010525 \\
        10 & total\_gu\_wobbles & 0.010238 \\
        11 & mre\_3p\_energy\_mean & 0.009645 \\
        12 & consecutive\_matches\_minus\_seed & 0.008837 \\
        13 & conservation\_range & 0.007518 \\
        14 & five\_prime\_flank\_conservation\_mean & 0.005568 \\
        15 & seed\_conservation\_median & 0.005387 \\
        16 & mirna\_3p\_energy\_mean & 0.005385 \\
        17 & binding\_type\_5mer.mismatch.3prime & 0.004369 \\
        18 & mirna\_seed\_energy\_mean & 0.003605 \\
        19 & binding\_type\_3prime.compensatory & 0.003533 \\
        20 & total\_matches & 0.002513 \\
        21 & mirna\_seed\_energy\_min & 0.002442 \\
        22 & mre\_5p\_energy\_max\_jump & 0.002433 \\
        23 & mirna\_seed\_RRY\_freq & 0.002424 \\
        24 & seed\_au\_content & 0.002269 \\
        25 & seed\_conservation\_max & 0.002258 \\
        26 & mre\_3p\_energy\_max\_jump & 0.001799 \\
        27 & binding\_type\_6mer.mirna.bulge.3prime & 0.001600 \\
        28 & mirna\_3p\_YYR\_freq & 0.001405 \\
        29 & downstream\_skewness & 0.001267 \\
        30 & mirna\_3p\_YRY\_freq & 0.001201 \\
        \bottomrule
    \end{tabular}
\end{table}

\par The features that seem to dominate the predictions are the Duplex features with the most important one being the Ensemble energy of the duplex. That is to be expected since the use of IntaRNA alone was the driver for the most prominent increase in metrics. That is also evident given that within the top 10 features, many are based on the binding type and the matches predicted within the seed, while energy features that are based on Turner stacking energies are also important.

\newpage
\begin{table}[H]
    \centering
    \caption{Permutation Feature Importance - Complete Results}
    \label{tbl:permutation_importance_complete}
    \footnotesize
    \begin{tabular}{llrr}
        \toprule
        \textbf{Rank} & \textbf{Feature} & \textbf{Permutation Importance} & \textbf{Std Dev} \\
        \midrule
        1 & Eall & 0.081164 & 0.006075 \\
        2 & seed\_matches\_2\_8 & 0.051036 & 0.005536 \\
        3 & gu\_wobbles\_in\_seed\_2\_8\_pos & 0.012984 & 0.003629 \\
        4 & total\_mre\_bulges & 0.004732 & 0.002487 \\
        5 & Eall1 & 0.004083 & 0.002012 \\
        6 & mre\_5p\_energy\_mean & 0.003394 & 0.001923 \\
        7 & binding\_type\_3prime.compensatory & 0.003389 & 0.003181 \\
        8 & mre\_3p\_energy\_mean & 0.003222 & 0.001212 \\
        9 & binding\_type\_seedless & 0.002955 & 0.001255 \\
        10 & consecutive\_matches\_minus\_seed & 0.002834 & 0.001218 \\
        11 & total\_gu\_wobbles & 0.002517 & 0.001481 \\
        12 & mirna\_seed\_energy\_min & 0.002209 & 0.000908 \\
        13 & binding\_type\_6mer.mismatch.3prime & 0.002101 & 0.000903 \\
        14 & seed\_au\_content & 0.001746 & 0.000644 \\
        15 & mirna\_seed\_energy\_volatility & 0.001266 & 0.000481 \\
        16 & mirna\_3p\_energy\_min & 0.001246 & 0.000517 \\
        17 & downstream\_skewness & 0.001226 & 0.000381 \\
        18 & mirna\_seed\_energy\_mean & 0.001180 & 0.000564 \\
        19 & mirna\_seed\_RRY\_freq & 0.001130 & 0.000510 \\
        20 & total\_matches & 0.000951 & 0.000741 \\
        21 & binding\_type\_5mer.mismatch.3prime & 0.000936 & 0.000776 \\
        22 & mirna\_seed\_5p\_terminal\_energy & 0.000882 & 0.000451 \\
        23 & mirna\_3p\_YYY\_freq & 0.000870 & 0.000418 \\
        24 & effective\_3prime\_matches & 0.000780 & 0.000549 \\
        25 & mirna\_3p\_energy\_std & 0.000571 & 0.000331 \\
        26 & mre\_3p\_energy\_max\_jump & 0.000444 & 0.000350 \\
        27 & mirna\_3p\_energy\_volatility & 0.000368 & 0.000294 \\
        28 & mirna\_seed\_energy\_range & 0.000187 & 0.000211 \\
        29 & binding\_type\_6mer.mirna.bulge.3prime & 0.000165 & 0.000232 \\
        30 & mre\_3p\_energy\_range & 0.000164 & 0.000156 \\
        \bottomrule
    \end{tabular}
\end{table}

\par Permutation Feature Importance seems to paint the same picture as IntaRNA based features dominate the top of the list as well validating that it was a sound choice to include such features for the target site prediction task. Unfortunately, although there was an attempt to extract sequence based features, due to the constraint that Machine Learning models outside of Deep Neural Networks need tabular data, some information is bound to have been lost. Since more advanced architectures can easily read such sequential data, it would be interesting as a future endeavor to incorporate sequence along with energy related features and train a more advanced model with the goal of evaluating if such features are already learned by sequence alone from such architectures.

\chapter{Discussion}
\par MicroRNAs partake in many physiological processes and their deregulation can be found within many disease states. Given their central role in post-transcriptional regulation and the cost of experimentally validating targets of these molecules, it is vital that algorithms for this task are developed so that the binding rules are elucidated across tissues and states. Recently many machine learning and Deep learning architectures aim to perform target site prediction, either on the MRE level or the gene level with varying degrees of success. Within the scope of this thesis, an MRE target site predictor was developed by examining the performance of Language models as well as Machine Learning ensembles, using the Average Precision score as the evaluation metric across unseen miRNA-MRE pairs containing both seen and unseen miRNA families for a more robust way to measure generalization. Inspired by MicroT-CNN, feature engineering for the Machine Learning ensembles utilized tools for thermodynamic feature extraction as well as structural features of the miRNA-MRE duplex. Sequence features were based on encoding nucleotides as Purines or Pyrimidines before constructing trimer frequencies in order to reduce noise and encoding dimers of nucleotides relative to their Turner stacking energies, while conservation features describing the distribution of the PhastCons470way scores were extracted from the vectors. 

The approach utilizing Machine Learning ensembles seems to yield superior results, given its rich feature space. Feature importance analysis showed that thermodynamic features along with duplex related ones are the drivers for model performance and generalization to unseen data, hinting that they may hold some promise in being used also as part of larger models to help the increase prediction quality. Given the complexity of current deep learning approaches, it is difficult to interpet their predictions and understand which known properties drive the classification. In fact due to the statistical nature of these models, the most widely used way to interpret them involves SHAP values, which while shedding some light into the black box, are not always interpetable. Tree based models although having the caveat of requiring extensive feature engineering, have the advantadge of being naturally interpetable via the thresholds they set. The ensemble developed in this thesis while interpeted in the scope of SHAP values, provides the capability of using each of the models of the ensemble separately with minimal loss in performance which allows one to use, for instance, the trained LightGBM and interpeting its thresholds for interaction classification.
...
The Github repository containing the code and the instructions to build the environment necessary to reproduce the \nameref{sec:Results} of this thesis exist at \href{https://github.com/RafailTn/msc-thesis/tree/main}{Msc-Thesis Github}
